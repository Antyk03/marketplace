...
export class MessaggiService {
  ...
  mostraMessaggioInformazioni(message: string): void {
  ...
  private openSnackBar(timeout: number, { message }: { message:string}, selector: string = 'app-root', backgroundColor?: string, textColor?: string) {
    if (this.opened) {
